...
$uid = $bc->getMyId();
$response = $bc->getTodoListsForPerson($uid);
// see the XML output
...
foreach($response['body']->{'todo-list'} as $list) {
	...
	foreach($list->{'todo-items'}->{'todo-item'} as $item) {
		...
		$the_list[] = $item_text . ' - ' . $list_title . '^'.$project_name.'^https://'.$sitename.'.basecamphq.com/todo_lists/'.$list_id.'^'.$item_due;
	}
}
sort($the_list);
?>
BEGIN:VCALENDAR
CALSCALE:GREGORIAN
X-WR-CALNAME:Basecamp Task List
...
<?php echo 'DUE:'.$pieces[3]; // piece2 ?>

BEGIN:VALARM
ACTION:AUDIO
TRIGGER:-PT15M
X-WR-ALARMUID:8C3C1EED-FBCA-4B04-9643-94ECE8AC9480
